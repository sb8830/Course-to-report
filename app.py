...
if uploaded_file:
    ...
    # Strip timestamps from ALL date columns
        ...
        for i in range(max_courses):
            if i > 0:
                if i < len(courses) and (i - 1) < len(courses):
                    try:
                        curr = pd.to_datetime(courses.loc[i, "startdate"], errors="coerce")
                        prev = pd.to_datetime(courses.loc[i - 1, "startdate"], errors="coerce")
